	...
	struct pn547_dev *pdev)
{
	...
	struct device_node *nfc_device_node;
	struct platform_device *nfc_pdev;
	struct pinctrl *nfc_pinctrl;

	nfc_device_node = of_parse_phandle(np, "i2c_node", 0);
	if (!IS_ERR_OR_NULL(nfc_device_node)) {
		nfc_pdev = of_find_device_by_node(nfc_device_node);
		nfc_pinctrl = devm_pinctrl_get_select(&nfc_pdev->dev, "i2c_sleep");
		if (IS_ERR(nfc_pinctrl))
			pr_info("nfc pin configuration is failed\n");
		else
			pr_info("nfc pin configuration is success\n");
	} else {
		pr_info("target does not use nfc pinctrl\n");
	}
