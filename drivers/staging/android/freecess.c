...
int mod_sendmsg(int type, int mod, int caller_pid, int target_uid, int flag)
{
	int ret, msg_len = 0;
...
{
	int ret = RET_OK;
	int caller_pid = task_tgid_nr(caller);
	...
	if (thread_group_is_frozen(p) && (target_pid != last_kill_pid)) {
		...
		ret = mod_sendmsg(MSG_TO_USER, MOD_SIG, caller_pid, uid, 0);
...
int binder_report(struct task_struct *caller, struct task_struct *p, int flag)
{
	...
	if(p)
		uid = task_uid(p).val;
	if(caller)
		caller_pid = task_tgid_nr(caller);
	
	walltime = ktime_to_us(ktime_get());
	if (p && thread_group_is_frozen(p)) {
		ret = mod_sendmsg(MSG_TO_USER, MOD_BINDER, caller_pid, uid, flag);
...
static void recv_handler(struct sk_buff *skb)
{
	...
	if (skb->len >= NLMSG_SPACE(0)) {
		...
		if (msglen >= (sizeof(struct kfreecess_msg_data))) {
			...
			switch (payload->type) {
			case LOOPBACK_MSG:
				mod_sendmsg(LOOPBACK_MSG, payload->mod, 0, 0, 0);
				break;
			default:
...
{
	int ret = RET_ERR;
	struct proc_dir_entry *freecess_modstat_entry;
	struct proc_dir_entry *freecess_window_stat_entry;
	int i;
	...
	if (!freecess_rootdir)
		...
		freecess_window_stat_entry = proc_create("windowstat", 0666, freecess_rootdir, &window_stat_proc_fops);
