	...
	if (sec_debug_reset_ex_info) {
		...
		if (p_ex_info->cpu == -1) {
			...
			snprintf(p_ex_info->task_name,
				sizeof(p_ex_info->task_name), "%s",
				current->comm);
			p_ex_info->ktime = local_clock();
			snprintf(p_ex_info->pc,
				sizeof(p_ex_info->pc), "%pS",
				(void *)regs->ARM_PT_REG_PC);
			snprintf(p_ex_info->lr,
				sizeof(p_ex_info->lr), "%pS",
				(void *)regs->ARM_PT_REG_LR);
...
static ssize_t ___build_root_init(char *str)
{
	char *st, *ed;
	ssize_t len;

	ed = strnstr(str, "/android/kernel", strlen("/android/kernel"));
	if (!ed || ed == str)
		return -EINVAL;
...
int __init sec_debug_summary_init(void)
{
	...
	/* TODO: temporary disable under bring-up */
