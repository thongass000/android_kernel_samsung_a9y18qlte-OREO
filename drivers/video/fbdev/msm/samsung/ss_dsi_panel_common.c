			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	/* To guarantee copr operation */
	mutex_init(&vdd_data.copr.copr_lock);
...
struct dsi_panel_cmds *get_panel_tx_cmds(struct mdss_dsi_ctrl_pdata *ctrl, enum mipi_samsung_tx_cmd_list cmd)
{
	...
	else if (cmd == TX_SELF_MASK_IMAGE || cmd == TX_SELF_ICON_IMAGE ||
			 cmd == TX_SELF_ACLOCK_IMAGE || cmd == TX_SELF_ICON_IMAGE) { /*make only once*/
		LCD_DEBUG("cmd(%d) name(%s)\n", cmd, vdd->dtsi_data[ndx].panel_tx_cmd_list[cmd][0].name);
		cmds = &vdd->dtsi_data[ndx].panel_tx_cmd_list[cmd][0];
...
#define MAX_DSI_FIFO_SIZE_HS_MODE	(200) /* AP limitation - SDM660 */
#define GCT_CMD_ALIGN (12)
				...
				int target, u8 *buffer, int len)
{
	...
	while (p_size < MAX_DSI_FIFO_SIZE_HS_MODE) {
		 if (len % p_size == 0) {
			payload_size = p_size;
		 }
		 p_size += CMD_ALIGN;
	...
	cmd_size = len / payload_size;

	tx_cmds->link_state = DSI_HS_MODE;
	tx_cmds->cmd_cnt = cmd_size;

	LCD_INFO("len=%d, cmd_cnt=%d, payload_size=%d\n", len, tx_cmds->cmd_cnt, payload_size);

	cmds = vzalloc(sizeof(struct dsi_cmd_desc) * tx_cmds->cmd_cnt);
	payload = vzalloc((payload_size + 1) * tx_cmds->cmd_cnt);
			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	LCD_INFO("vdd->multires_stat.is_support = %d\n", vdd->multires_stat.is_support);
	...
	LCD_INFO("vdd->act_clock.is_support = %d\n", vdd->act_clock.is_support);
...
void set_normal_br_values(struct samsung_display_driver_data *vdd, int ndx)
{
	...
	LCD_DEBUG("table size (%d) max_lv (%d) \n", table->tab_size, table->max_lv);
